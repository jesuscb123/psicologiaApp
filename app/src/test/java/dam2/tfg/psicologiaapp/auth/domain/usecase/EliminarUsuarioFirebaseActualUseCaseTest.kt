...
class EliminarUsuarioFirebaseActualUseCaseTest {

    @Test
    fun `invoke debe propagar exito del repositorio`() = runTest {
        val expected = Unit
        val repo = object : FakeAuthRepository() {
            override suspend fun eliminarUsuarioActual() = Result.success(expected)
        }
        val useCase = EliminarUsuarioFirebaseActualUseCase(repo)
    ...
    fun `invoke debe propagar fallo del repositorio`() = runTest {
        val repo = object : FakeAuthRepository() {
            override suspend fun eliminarUsuarioActual() =
