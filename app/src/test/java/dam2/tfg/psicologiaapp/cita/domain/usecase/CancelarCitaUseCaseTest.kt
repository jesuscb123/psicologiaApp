...
class CancelarCitaUseCaseTest {
    ...
    fun `invoke debe propagar exito del repositorio`() = runTest {
        ...
        val repo = object : FakeCitaRepository() {
            override suspend fun cancelarCita(citaId: Long) = Result.success(expected)
        }
        val useCase = CancelarCitaUseCase(repo)
        val resultado = useCase(citaId = 1L)
    ...
    fun `invoke debe propagar fallo del repositorio`() = runTest {
        val repo = object : FakeCitaRepository() {
            override suspend fun cancelarCita(citaId: Long) =
