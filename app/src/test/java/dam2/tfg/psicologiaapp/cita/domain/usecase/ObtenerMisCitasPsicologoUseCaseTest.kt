...
class ObtenerMisCitasPsicologoUseCaseTest {
    ...
    fun `invoke debe propagar exito del repositorio`() = runTest {
        ...
        val repo = object : FakeCitaRepository() {
            override suspend fun getMisCitasPsicologo() = Result.success(expected)
        }
        val useCase = ObtenerMisCitasPsicologoUseCase(repo)
    ...
    fun `invoke debe propagar fallo del repositorio`() = runTest {
        val repo = object : FakeCitaRepository() {
            override suspend fun getMisCitasPsicologo() =
