...
class ActualizarNotaUseCaseTest {
    ...
    fun `invoke debe propagar exito del repositorio`() = runTest {
        ...
        val repo = object : FakeNotaRepository() {
            override suspend fun actualizarNota(notaId: Long, asunto: String, descripcion: String) = Result.success(expected)
        }
        val useCase = ActualizarNotaUseCase(repo)
        val resultado = useCase(notaId = 1L, asunto = "A", descripcion = "D")
    ...
    fun `invoke debe propagar fallo del repositorio`() = runTest {
        val repo = object : FakeNotaRepository() {
            override suspend fun actualizarNota(notaId: Long, asunto: String, descripcion: String) =
