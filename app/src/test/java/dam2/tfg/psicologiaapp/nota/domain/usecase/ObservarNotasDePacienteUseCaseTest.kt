...
class ObservarNotasDePacienteUseCaseTest {
    ...
    fun `invoke debe emitir valores del repositorio`() = runTest {
        ...
        val repo = object : FakeNotaRepository() {
            override fun observarNotasDePaciente(pacienteId: Long) = flowOf(expected)
        }
        val actual = ObservarNotasDePacienteUseCase(repo)(pacienteId = 1L).first()
