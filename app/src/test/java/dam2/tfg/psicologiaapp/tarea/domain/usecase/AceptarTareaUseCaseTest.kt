...
class AceptarTareaUseCaseTest {
    ...
    fun `invoke debe propagar exito del repositorio`() = runTest {
        ...
        val repo = object : FakeTareaRepository() {
            override suspend fun aceptarTarea(tareaId: Long) = Result.success(expected)
        }
        val resultado = AceptarTareaUseCase(repo)(tareaId = 1L)
    ...
    fun `invoke debe propagar fallo del repositorio`() = runTest {
        val repo = object : FakeTareaRepository() {
            override suspend fun aceptarTarea(tareaId: Long) =
