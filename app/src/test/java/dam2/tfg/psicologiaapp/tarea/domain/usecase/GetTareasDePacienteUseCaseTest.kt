...
class GetTareasDePacienteUseCaseTest {
    ...
    fun `invoke debe propagar exito del repositorio`() = runTest {
        ...
        val repo = object : FakeTareaRepository() {
            override suspend fun getTareasDePaciente(pacienteId: Long) = Result.success(expected)
        }
        val resultado = GetTareasDePacienteUseCase(repo)(pacienteId = 1L)
    ...
    fun `invoke debe propagar fallo del repositorio`() = runTest {
        val repo = object : FakeTareaRepository() {
            override suspend fun getTareasDePaciente(pacienteId: Long) =
